...
export async function GET(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    const { id } = params
    const response = await fetch(`${LOGISTICS_BACKEND_URL}/api/logistics/schedules/${id}/port-candidates`, {
      headers: {
    ...
    if (!response.ok) {
      return NextResponse.json({ error: "Failed to fetch port candidates" }, { status: response.status })
    }
