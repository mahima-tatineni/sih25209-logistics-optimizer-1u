...
export async function GET(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    const { id } = params
    const response = await fetch(`${LOGISTICS_BACKEND_URL}/api/logistics/schedules/${id}/scenarios`, {
      headers: {
    ...
    if (!response.ok) {
      return NextResponse.json({ error: "Failed to fetch scenarios" }, { status: response.status })
    }
