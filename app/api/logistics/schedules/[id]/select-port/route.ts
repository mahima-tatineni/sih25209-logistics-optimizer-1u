...
export async function POST(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const response = await fetch(`${LOGISTICS_BACKEND_URL}/api/logistics/schedules/${id}/select-port`, {
      method: "POST",
      headers: {
    ...
    if (!response.ok) {
      return NextResponse.json({ error: "Failed to select port" }, { status: response.status })
    }
